...
# # --- Configuration ---
# DEVICE = "cuda:0"
# # YOLO_WEIGHTS_PATH = "/DATA/yolo_on_brain/runs/detect/brain_yolov8m3/weights/best.pt"
# YOLO_WEIGHTS_PATH = "/DATA/yolo_on_brain/runs/detect/brain_yolov8m_less_fn2/weights/best.pt"
# MEDSAM_CKPT_PATH = "/DATA/medsam_inference/medsam_vit_b.pth"
# # ROOT_FOLDER = "/DATA/yolo_on_brain/output-african"
# ROOT_FOLDER = "/DATA/yolo_on_brain/output-shuffled-t1ce-t2w-flair"
# # Note: Ensure this path exists and is writable
# OUTPUT_ROOT = "/DATA/yolo_on_brain/rohit_seg"
# BUFFER_BOUNDING_BOX = 10  # pixels
...
#             results = yolo_model(image_path, conf=0.1, iou=0.5,classes=[0]) # Adjusted confidence and IoU thresholds
...
#                 x1, y1, x2, y2 = box
#                 x1 = max(x1 - BUFFER_BOUNDING_BOX, 0)
#                 y1 = max(y1 - BUFFER_BOUNDING_BOX, 0)
#                 x2 = min(x2 + BUFFER_BOUNDING_BOX, W - 1)
#                 y2 = min(y2 + BUFFER_BOUNDING_BOX, H - 1)
#                 box_1024 = [x1 * scale_x, y1 * scale_y, x2 * scale_x, y2 * scale_y]
#                 box_1024_list.append(box_1024)

#             masks = [medsam_inference(medsambox_1024_model, img_embed, [], H, W)
...
#         # --- Get original affine and header ---
...
YOLO_WEIGHTS_PATH = "/DATA/yolo_on_brain/runs/detect/brain_yolov8m3/weights/best.pt"
MEDSAM_CKPT_PATH = "/DATA/medsam_inference/medsam_vit_b.pth"
SINGLE_CASE_FOLDER = "/DATA/yolo_on_brain/output-shuffled-t1ce-t2w-flair/shuffled_0012"  # <<< Set your case folder
OUTPUT_ROOT = "/DATA/yolo_on_brain/rohit_seg"
BUFFER_BOUNDING_BOX = 10  # pixels
...
image_dir = os.path.join(SINGLE_CASE_FOLDER, "images/combined")

if not os.path.exists(image_dir):
    logging.error(f"No images/combined directory in {SINGLE_CASE_FOLDER}")
    exit(1)
...
try:
    ...
    logging.error(f"Could not list files in {image_dir}: {e}")
    exit(1)
...
pred_masks = []
logging.info(f"Processing single folder: {image_dir}")
...
for image_path in image_paths:
    try:
        ...
        results = yolo_model(image_path, conf=0.1, iou=0.5, classes=[0])
        pred_boxes = results[0].boxes.xyxy.cpu().numpy()
...
if not pred_masks:
    ...
    exit(1)
try:
    pred_masks.sort()  # Sort by slice index
    output_case_dir = os.path.join(OUTPUT_ROOT, os.path.basename(SINGLE_CASE_FOLDER), "png_slices")
    os.makedirs(output_case_dir, exist_ok=True)

    for slice_index, mask in pred_masks:
        output_slice_path = os.path.join(output_case_dir, f"slice_{slice_index:04d}.png")
        cv2.imwrite(output_slice_path, mask * 255)  # multiply by 255 to save as visible binary image
        logging.info(f"Saved slice: {output_slice_path}")

    logging.info(f"All slices saved to {output_case_dir}")
except Exception as e:
    logging.error(f"Failed to save PNG slices: {e}")
